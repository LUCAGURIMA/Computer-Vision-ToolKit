...
class CaptureManagerAPI:
    ...
    def _log(self, level: str, msg: str):
        try:
            ...
            if callable(logger_method):
                logger_method(f'[CaptureManagerAPI] {msg}')
            else:
                log.info(f'[CaptureManagerAPI] {msg}')
        except Exception:
            log.info(f'[CaptureManagerAPI] {msg}')
    ...
    async def capture_image(self) -> CaptureResponse:
        try:
            ...
            if not result:
                return CaptureResponse(success=False, message='Falha ao capturar imagem')
            processed = self.manager.process_captured_image(result)
            if not processed:
                return CaptureResponse(success=False, message='Falha ao processar imagem')
            image = processed.get('image')
            if image is None:
                return CaptureResponse(success=False, message='Nenhuma imagem após processamento')
            rgb_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            _, buffer = cv2.imencode('.png', rgb_image)
            image_b64 = base64.b64encode(buffer).decode('utf-8')
            return CaptureResponse(success=True, message='Captura realizada com sucesso', data={'shape': processed.get('shape'), 'crop_applied': processed.get('crop_applied', False), 'size_mb': image.nbytes / (1024 * 1024), 'image_b64': image_b64, 'timestamp': processed.get('timestamp', '')})
        except Exception as e:
            self._log('error', f'Erro ao capturar: {e}')
            return CaptureResponse(success=False, message=f'Erro: {str(e)}')
    ...
    def set_crop_settings(self, request: CropSettingsRequest) -> CaptureResponse:
        try:
            ...
            return CaptureResponse(success=True, message=f"Crop {('habilitado' if request.enabled else 'desabilitado')}", data={'crop_enabled': request.enabled, 'crop_bbox': request.bbox})
        except Exception as e:
    ...
    def reset_crop(self) -> CaptureResponse:
        try:
            ...
            return CaptureResponse(success=True, message='Crop removido', data={'crop_enabled': False})
        except Exception as e:
    ...
    def set_inspection(self, request: InspectionRequest) -> CaptureResponse:
        try:
            ...
            return CaptureResponse(success=True, message=f"Inspeção {('habilitada' if request.type else 'desabilitada')}", data={'inspection_type': request.type})
        except Exception as e:
    ...
    def get_inspection_status(self) -> CaptureResponse:
        try:
            ...
            return CaptureResponse(success=True, data={'inspection_type': insp_type, 'enabled': insp_type is not None})
        except Exception as e:
    ...
    def get_image_info(self) -> CaptureResponse:
        try:
            ...
            return CaptureResponse(success=True, data={'shape': info.get('shape'), 'dtype': info.get('dtype'), 'size_mb': info.get('size_mb', 0), 'crop_applied': info.get('crop_applied', False), 'crop_bbox': info.get('crop_bbox'), 'status': info.get('status', 'OK')})
        except Exception as e:
    ...
    def get_status(self) -> CaptureResponse:
        try:
            info = self.manager.get_image_info()
            insp_type = self.manager.should_inspect_after_capture()
            return CaptureResponse(success=True, data={'image': info, 'crop': {'enabled': self.manager.crop_enabled, 'bbox': self.manager.crop_bbox}, 'inspection': {'type': insp_type, 'enabled': insp_type is not None}})
        except Exception as e:
    ...
    def clear_images(self) -> CaptureResponse:
        try:
            ...
            return CaptureResponse(success=True, message='Imagens limpas')
        except Exception as e:
    ...
    def apply_profile(self, profile_name: str) -> CaptureResponse:
        try:
            ...
            if success:
                return CaptureResponse(success=True, message=f"Perfil '{profile_name}' aplicado", data={'profile': profile_name})
            else:
                return CaptureResponse(success=False, message=f"Falha ao aplicar perfil '{profile_name}'")
        except Exception as e:
